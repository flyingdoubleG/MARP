...
class ModelEvaluator():
    ...
    def parse_multiple_choice(self, response):
        """
        Parses the multiple choices from the evaluator's response.
        ...
        try:
            ...
            for idx, line in enumerate(lines):
                choice = line[-1]
                if choice is None:
                    raise ValueError(f"{line} not contain choice")
                if idx < 3:
                    if choice == 'A': # Story 1 better
                        llmScore1.append(1)
                        llmScore2.append(0)
                    elif choice == 'B': # Story 2 better
                        llmScore1.append(0)
                        llmScore2.append(1)
                    elif choice == 'C': # Both are about equally good
                        llmScore1.append(1)
                        llmScore2.append(1)
    ...
    def evaluate_stories(self, premises, stories1, stories2, scores1=None, scores2=None):
        """
            ...
            while repeat_query and max_tries > 0:
                ...
                try:
                    if self.dataset_name == "hanna":
                        ...
                        llmScore1, llmScore2 = self.parse_multiple_choice(response)
                    repeat_query = False
                except:
                    repeat_query = True
                    max_tries -= 1
    ...
    def evaluatePrefix(self):
        ...
        train_set, test_set = loader.splitTrainTest(prompt2Idx, idx2Prompt, prompt2Scores, prompt2Stories, self.num_prompts_eval)

        return writers, train_set, test_set

    def evaluate(self):  # evaluate 2 stories - q
        writers, train_set, test_set = self.evaluatePrefix()
    ...
    def evaluateModels(self): # evaluate 1 story - q, rank models
        """
