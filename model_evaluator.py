...
class ModelEvaluator():
    ...
    def compute_model_eval_acc(self, scores1, scores2, llmScores1, llmScores2, writer1, writer2, prompts):
        ...
        for i in range(len(scores1)):
            if (scores1[i] > scores2[i]) and (llmScores1[i] > llmScores2[i]):
                acc += 1
                comparison_tasks.append(ComparisonTask(evaluator_name, "win", "win", writer1, writer2, prompts[i], self.initial_task_id))
            elif (scores1[i] <= scores2[i]) and (llmScores1[i] <= llmScores2[i]):
                acc += 1
                comparison_tasks.append(ComparisonTask(evaluator_name, "lose", "lose", writer1, writer2, prompts[i], self.initial_task_id))
            elif (scores1[i] > scores2[i]) and (llmScores1[i] <= llmScores2[i]):
                comparison_tasks.append(ComparisonTask(evaluator_name, "win", "lose", writer1, writer2, prompts[i], self.initial_task_id))
            else:
                comparison_tasks.append(ComparisonTask(evaluator_name, "lose", "win", writer1, writer2, prompts[i], self.initial_task_id))
    ...
    def evaluate_stories(self, premises, stories1, stories2, writer1, writer2):
        """
        ...
        for i in tqdm(range(len(stories1)), desc=f"{writer1} vs {writer2}, premise {counter}", leave=False):
            ...
            # If the LLM scores for the current pair of stories have already been evaluated, use the stored scores
            if self.llm_labels[premise][writer1][writer2] is not None:
                llmScore1 = self.llm_labels[premise][writer1][writer2][0]
                llmScore2 = self.llm_labels[premise][writer1][writer2][1]
            ...
            # global CASE_NUM
            # if randint(0, CASE_NUM) == CASE_NUM // 2:
            #     failed = True
            if failed:
    ...
    def load_llm_labels(self, writers, prompt2Idx):
        if self.llm_labels is None:
            ...
            for prompt in list(prompt2Idx.keys())[:self.num_prompts_eval]:
                for writer1 in writers:
                    for writer2 in writers:
                        self.llm_labels[prompt][writer1][writer2] = None
        ...
        for i in range(len(writers)):
            ...
            for j in range(i+1, len(writers)):
                ...
                for _ in range(self.eval_rounds):
                    tmp_llmScores1, tmp_llmScores2 = self.evaluate_stories(premises, stories1, stories2, writer1, writer2)
                    llmScores1 += tmp_llmScores1
                    llmScores2 += tmp_llmScores2

                    # Bidirectional evaluation
                    if self.bidir_eval:
                        tmp_llmScores2, tmp_llmScores1 = self.evaluate_stories(premises, stories2, stories1, writer2, writer1)
                        llmScores2 += tmp_llmScores2
                        llmScores1 += tmp_llmScores1
