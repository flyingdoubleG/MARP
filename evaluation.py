...
'''
def evaluateModels(model, dataset_name, filepath, num_prompts_eval=3, num_categories=1, 
                   bidir_eval=False, eval_rounds=1):
    """
    ...
    for writer in writers:
        human_scores[writer] = 0.0

    # Collect total human-evaluated scores over selected prompts for each writer-model
    for writer in writers:
        for prompt in trainPrompt2Idx:
            human_scores[writer] += trainPrompt2Scores[prompt][writer]

    # Collect total scores evaluated by the scorer model over selected prompts for each writer model
    model_scores = {}
'''
...
if __name__ == '__main__':
    ...
    #                            num_prompts_eval=2, num_categories=6, bidir_eval=False, eval_rounds=1)
    ...
    evaluator = ModelEvaluator('anyscale/mistralai/Mistral-7B-Instruct-v0.1', 'hanna', 'hanna/hanna_stories_annotations.csv', 
                               num_prompts_eval=2, num_categories=6, bidir_eval=True, 
                               eval_rounds=1)
